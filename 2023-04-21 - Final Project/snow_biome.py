...
def main():
    ...
    for point in buildRect.outline:
        ...
        for y in range(height, height + 5):
            ...
            editor.placeBlock(addY(point, y+1), Block("snow_block"))
            editor.placeBlock(addY(point, y), Block("dark_oak_planks"))
    ...
    igloo_structure_width = get_igloo_dimensions()
    ...
    for _ in range(num_igloo_structures):
        random_center = generate_random_position(igloo_structure_width)
        local_pos= getlocal(random_center)
        print(local_pos)
        # townhall(editor,random_center,biome,grid,townhall_structure_width)
        is_overlap=will_overlap(grid,local_pos,igloo_structure_width,igloo_structure_width)
        print(is_overlap)
